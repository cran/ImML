...
ImML <- function(data, plotit=TRUE, setseed=NULL, verbose=FALSE, ...) {
  ...
  Plot = ggplot(plotData, aes(x = .data$x)) +
    geom_point(aes(y = .data$yLR, color = "Linear Regression")) +
    geom_point(aes(y = .data$yDT, color = "Decision Tree")) +
    geom_point(aes(y = .data$yRF, color = "Random Forest")) +
    geom_point(aes(y = .data$ySVM, color = "Support Vector Machine")) +
